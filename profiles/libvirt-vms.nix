...
let
  mkVM = { name, user, sshKey, cpus, memory, diskSize, osVariant ? "ubuntu24.04" }:
  let
  ...
  options.virtualMachines = lib.mkOption {
    type = lib.types.attrsOf (lib.types.submodule ({ name, config, ... }: {
      options = {
        ...
        cpus = lib.mkOption { type = lib.types.int; default = 2; };
        memory = lib.mkOption { type = lib.types.int; }; # в МБ
        diskSize = lib.mkOption { type = lib.types.int; }; # в ГБ
        osVariant = lib.mkOption { type = lib.types.str; default = "ubuntu24.04"; };
      };
    }));
  };

  config = lib.mkMerge (lib.mapAttrsToList (vmName: vmConfig: 
    mkVM {
      name = vmName;
      user = vmConfig.user;
